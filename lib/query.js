 ...
 * @param {Object} modification - modifications to perform on the found documents
 * @param {Object} options - options object
 * @api public
 */

Query.prototype.findAndModify = function (query, sort, modification, options) {
	let Model = this.model;
	...
	// findAndModify
	return this.collection.then(function (collection) {
		return collection.findAndModify(query, sort, modification, options);
	}).then(function(doc) {
		if (typeof doc !== 'undefined' &&
			doc.ok === 1 &&
			doc.lastErrorObject.n === 1) {
			return new Model(doc.value);
		}
	});
};
