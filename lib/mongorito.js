...
class EnforcerError extends Error {
  constructor(){
    super();
    this.name = 'EnforcerError';
  }
}
...
let mongoritoInstance = Symbol();
const MONGORITO_ENFORCER = Symbol();
class Mongorito {
  constructor(enforcer){
    if(MONGORITO_ENFORCER !== enforcer) {
      throw new EnforcerError('Cannot directly construct a Mongorito');
    }
    this._collections = {};
  }

  static get instance(){
    if(!this[mongoritoInstance]){
      this[mongoritoInstance] = new Mongorito(MONGORITO_ENFORCER);
    }
    return this[mongoritoInstance];
  }
  /**
  ...
  static connect () {
    ...
    return new Promise(function(resolve, reject){
      MongoClient.connect(urls[0], options).then(function(db){
        if(!Mongorito.instance.db) {
          Mongorito.instance.db = db;
        }
        resolve(db);
  ...
  static disconnect () {
    return Mongorito.instance.db.close();
  }
  ...
  static _collection (db, name) {
    ...
    let collections = Mongorito.instance._collections[url];

    if (!collections) {
      collections = Mongorito.instance._collections[url] = {};
    }
...
class Model {
  ...
  get _db () {
    ...
    return this.db || Mongorito.instance.db;
  }
  ...
  * save (options) {
    ...
    let populate = options.populate || emptyObject;
    let keys = Object.keys(populate);

    let self = this;

    keys.forEach(function (key) {
      let value = populate[key];
  ...
  * inc (props, options) {
  ...
  static get _db () {
    ...
    return this.prototype.db || Mongorito.instance.db;
  }
  ...
  static * index () {
    return yield this._collection.ensureIndex.apply(this._collection, arguments);
  }
  ...
  static * indexes () {
    let indexCursor = this._collection.listIndexes.apply(this._collection, arguments);
    return yield indexCursor.toArray();
  }
}
