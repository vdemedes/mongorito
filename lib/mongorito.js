...
Model.push = function (id, attribute, child) {
	return new Query(this._collection(), this).findOne({ _id: id })
		.then(post => {
			if (!post) {
				throw new Error('No posts with that id');
			}
			if (post.get(attribute)) {
				post.get(attribute).push(child.get('_id'));
				return post.save();
			}
		});
};
